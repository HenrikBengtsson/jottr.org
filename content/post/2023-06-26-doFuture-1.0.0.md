...
instead. So, why is that better?  It is because: 
 ...
 4. `%dofuture%` is guaranteed to always parallelize via the
...
backends developed later on will automatically work with your
...
parallel random number generation (RNG).  Statistically sound parallel
RNG is critical for many statistical analyses.  If not done, then the
results can be biased and incorrect conclusions might be drawn.  The
...
However, the future framework will detect if we forget to
...
list(seed = TRUE)` to declare that we need to draw random numbers in
parallel, i.e.
...
This makes sure that statistically sound random numbers are generated.
...
`%dofuture%` instead, then it only takes a few changes, which are
